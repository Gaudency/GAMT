...
<body class="bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200 min-h-full transition-colors duration-300">
    ...
    <div class="flex mt-16">
        ...
        <div class="flex-1 p-6">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                <div class="mb-6">
                    <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Registrar Nuevo Préstamo Suelto</h1>
                </div>
                ...
                <form action="{{ route('loose-loans.store') }}" method="POST" class="space-y-6">
                    @csrf

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="folder_code" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Código digital o fisico de la Carpeta</label>
                            <input type="text" name="folder_code" id="folder_code" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800 shadow-sm focus:border-blue-500 focus:ring-blue-500" required value="{{ old('folder_code') }}">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="sheets_count" id="sheets_count" min="1" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800 shadow-sm focus:border-blue-500 focus:ring-blue-500" required value="{{ old('sheets_count') }}">
                        </div>

                        <div>
                            <label for="lender_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nombre del Prestador</label>
                            <div class="relative">
                                <input type="text" name="lender_name" id="lender_name" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800 shadow-sm focus:border-blue-500 focus:ring-blue-500 pr-10" required value="{{ old('lender_name') }}">
                        ...
                        <div>
                            <label for="loan_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Fecha y Hora de Préstamo</label>
                            <input type="datetime-local" name="loan_date" id="loan_date" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800 shadow-sm focus:border-blue-500 focus:ring-blue-500" required value="{{ old('loan_date', date('Y-m-d\TH:i')) }}">
                        </div>

                        <div>
                            <label for="return_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Fecha y Hora de Devolución</label>
                            <input type="datetime-local" name="return_date" id="return_date" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800 shadow-sm focus:border-blue-500 focus:ring-blue-500" required value="{{ old('return_date', date('Y-m-d\TH:i', strtotime('+7 days'))) }}">
                        </div>
                    </div>

                    <div>
                        <label for="description" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Descripción</label>
                        <textarea name="description" id="description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description') }}</textarea>
                    ...
                    <div>
                        ...
                        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">La firma digital es opcional.</p>
                    </div>

                    <div class="mt-4 p-4 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg">
                        <div class="flex items-start">
                            <div class="flex items-center h-5">
                                <input id="terms_confirmation" name="terms_confirmation" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500" value="1" required>
                            </div>
                            <div class="ml-3 text-sm">
                                ...
                                <p class="text-gray-500 dark:text-gray-400">Esta confirmación es obligatoria para crear el préstamo.</p>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        document.querySelector('form').addEventListener('submit', function(e) {
            ...
            const termsConfirmed = document.getElementById('terms_confirmation').checked;

            // Siempre verificamos que el checkbox esté marcado (aunque ya lo hace HTML con required)
            if (!termsConfirmed) {
                e.preventDefault();
            ...
            try {
                // Solo procesar la firma si no está vacía
                if (!signaturePad.isEmpty()) {
                    // Obtener la firma como imagen base64 con calidad reducida para menor tamaño
