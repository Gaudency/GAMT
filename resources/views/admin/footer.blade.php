<footer class="w-full bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 mt-auto py-6 transition-colors duration-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row justify-between items-center">
            <div class="mb-4 md:mb-0">
                <p class="text-sm text-gray-600 dark:text-gray-300">
                    2025 &copy; Into your Municipality <span class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-violet-600 font-bold">@Tomave</span>
                </p>
            </div>
            <div class="flex space-x-4">
                <!-- Sitio Web -->
                <a href="#" class="text-blue-500 hover:text-blue-600 dark:text-blue-400 dark:hover:text-blue-300 transition-colors duration-300">
                    <i class="fas fa-globe text-lg"></i>
                </a>

                <!-- Facebook -->
                <a href="#" class="text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 transition-colors duration-300">
                    <i class="fab fa-facebook-f text-lg"></i>
                </a>

                <!-- Instagram -->
                <a href="#" class="text-pink-500 hover:text-pink-600 dark:text-pink-400 dark:hover:text-pink-300 transition-colors duration-300">
                    <i class="fab fa-instagram text-lg"></i>
                </a>

                <!-- X (Twitter) -->
                <a href="#" class="text-gray-800 hover:text-gray-900 dark:text-gray-200 dark:hover:text-white transition-colors duration-300">
                    <span class="font-bold text-lg">X</span>
                </a>

                <!-- WhatsApp -->
                <a href="#" class="text-green-500 hover:text-green-600 dark:text-green-400 dark:hover:text-green-300 transition-colors duration-300">
                    <i class="fab fa-whatsapp text-lg"></i>
                </a>

                <!-- TikTok -->
                <a href="#" class="text-gray-800 hover:text-black dark:text-gray-200 dark:hover:text-white transition-colors duration-300">
                    <i class="fab fa-tiktok text-lg"></i>
                </a>
            </div>
        </div>
    </div>
</footer>
