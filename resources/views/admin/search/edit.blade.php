...
<body class="bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-800 dark:text-gray-200">
    ...
    <div class="max-w-3xl mx-auto px-4 py-10">
        <h1 class="text-3xl md:text-4xl font-bold text-center mb-8 text-gray-800 dark:text-white">Editar Préstamo</h1>
        
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 md:p-8 border border-gray-200 dark:border-gray-700">
            <form action="{{ route('loan.update', $loan->book_id) }}" method="POST">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <input type="number" 
                           name="page_count" 
                           id="page_count" 
                           value="{{ $loan->page_count }}"
                           required
                           class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 focus:border-blue-500 focus:ring-blue-500 dark:focus:border-blue-400 dark:focus:ring-blue-400">
                </div>
                
                <div class="mb-8">
                    <label for="is_borrowed" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        Estado
                    </label>
                    <select name="is_borrowed" 
                            ...
                            required
                            class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 focus:border-blue-500 focus:ring-blue-500 dark:focus:border-blue-400 dark:focus:ring-blue-400">
                        <option value="1" {{ $loan->is_borrowed ? 'selected' : '' }}>Prestado</option>
                        <option value="0" {{ !$loan->is_borrowed ? 'selected' : '' }}>Disponible</option>
                    </select>
                </div>
                
                <div class="flex justify-center space-x-4">
                    <a href="{{ url()->previous() }}" 
                       class="px-6 py-3 bg-gray-500 hover:bg-gray-600 text-white font-medium rounded-lg shadow-md hover:shadow-lg transition duration-300">
                        Cancelar
                    </a>
                    <button type="submit" 
